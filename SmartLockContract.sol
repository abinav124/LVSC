...
contract VSCSmartLock is Ownable, Pausable {
    ...
    mapping(address=>mapping(uint256 => Users)) public user; 
    mapping(address=>uint256) public count; 
    event lockeventIN(address account,uint256 amount, uint timestamp); 
    event lockeventOUT(address account, uint256 amount, uint timestamp);

     constructor (address _token)  {
        token = IERC20(_token);
    }

    // receive VSC directly 
    ...
    user[_msgSender()][count[_msgSender()]]=newuser;
    count[_msgSender()]= count[_msgSender()].add(1);
    token.mint(_msgSender(),msg.value);
    token.updateLockStatus(_msgSender(),true);
    emit lockeventIN(_msgSender(),msg.value,block.timestamp);
    }

    function updateLockPeriod(uint256 _seconds) public onlyOwner {
        LockPeriod = _seconds;
    }
    ...
    function withdraw(uint256 _id) public whenNotPaused isContractCheck(msg.sender){
        ...
        token.transferFrom(_msgSender(),address(this),user[_msgSender()][_id].amount);
